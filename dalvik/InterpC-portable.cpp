...
uint32_t total_pointer=0;
uint32_t id=1;
static char dumppath[100]={0};
static char dumppointer[100]={0};
...
void writeULeb128(uint8_t ** ptr, uint32_t data)
{
    while (true) {
...
uint8_t* codeitemend(const u1** pData)
{
...
uint8_t* rewrite_ClassData(DexClassData *pData, int& len)
{
    len=0;
    ...
    writeULeb128(&store,pData->header.staticFieldsSize);
    writeULeb128(&store,pData->header.instanceFieldsSize);
    writeULeb128(&store,pData->header.directMethodsSize);
    writeULeb128(&store,pData->header.virtualMethodsSize);

    if (pData->staticFields) {
        for (uint32_t i = 0; i < pData->header.staticFieldsSize; i++) {
	        if(i<1){
            	writeULeb128(&store,pData->staticFields[i].fieldIdx);
            	writeULeb128(&store,pData->staticFields[i].accessFlags);
	        }else{
		        writeULeb128(&store,pData->staticFields[i].fieldIdx-pData->staticFields[i-1].fieldIdx);
            	writeULeb128(&store,pData->staticFields[i].accessFlags);
	        }
        }
    }
    ...
    if (pData->instanceFields) {
        for (uint32_t i = 0; i < pData->header.instanceFieldsSize; i++) {
	        if(i<1){
            	writeULeb128(&store,pData->instanceFields[i].fieldIdx);
            	writeULeb128(&store,pData->instanceFields[i].accessFlags);
	        }else{
		        writeULeb128(&store,pData->instanceFields[i].fieldIdx-pData->instanceFields[i-1].fieldIdx);
            	writeULeb128(&store,pData->instanceFields[i].accessFlags);
	        }
        }
    }
    ...
    if (pData->directMethods) {
        for (uint32_t i=0; i<pData->header.directMethodsSize; i++) {
	        if(i<1){
            	writeULeb128(&store,pData->directMethods[i].methodIdx);
            	writeULeb128(&store,pData->directMethods[i].accessFlags);
            	writeULeb128(&store,pData->directMethods[i].codeOff);
	        }else{
		        writeULeb128(&store,pData->directMethods[i].methodIdx-pData->directMethods[i-1].methodIdx);
            	writeULeb128(&store,pData->directMethods[i].accessFlags);
            	writeULeb128(&store,pData->directMethods[i].codeOff);
	        }
        }
    ...
    if (pData->virtualMethods) {
        for (uint32_t i=0; i<pData->header.virtualMethodsSize; i++) {
	        if(i<1){
            	writeULeb128(&store,pData->virtualMethods[i].methodIdx);
            	writeULeb128(&store,pData->virtualMethods[i].accessFlags);
            	writeULeb128(&store,pData->virtualMethods[i].codeOff);
	        }else{
		        writeULeb128(&store,pData->virtualMethods[i].methodIdx-pData->virtualMethods[i-1].methodIdx);
            	writeULeb128(&store,pData->virtualMethods[i].accessFlags);
            	writeULeb128(&store,pData->virtualMethods[i].codeOff);
	        }
        }
...
void getClassData(DvmDex* pDvmDex,int idx,FILE *fp1,FILE *fp2,Object *loader,Thread* self)
{
	...
	const char *descriptor = dexGetClassDescriptor(pDexFile,pClassDef);
	...
	if(clazz){
	    ...
	    if (pClassData->directMethods) {
		    for (int i = 0; i < (int) pClassData->header.directMethodsSize; i++) {
			    ALOGI("directMethodsSize is %x i is %x method is %x",(int) pClassData->header.directMethodsSize,i,int(&pClassData->directMethods[i]));
			    //ALOGI("pDexFile addr is %x",int(pDexFile));
			    Method *method = &(clazz->directMethods[i]);
			    const DexCode* pCode = (DexCode*)((const u1*)method->insns-16);
			    uint8_t *Code_star=(uint8_t *)pCode;
			    pClassData->directMethods[i].codeOff = total_pointer;
			    //ALOGI("total_pointer is %x",total_pointer);
			    ...
			    if (!method->insns||ac&ACC_NATIVE) {
                    if (pClassData->directMethods[i].codeOff) {
                        pClassData->directMethods[i].accessFlags=ac;
			    ...
			    if(pCode){
				    ALOGI("method is %x",(int)method);
				    ALOGI("pc is %x",(int)method->insns);
				    dvmInterpret(self, method, &retval);
				    ALOGI("zhixing wancheng!");
				    need_extra = true;
				    int total_length;
				    ...
				    if(pCode->triesSize){
                        ...
                        uint8_t * tail=codeitemend(phandler);
                        total_length = (int)(tail-(uint8_t*)pCode);
					//ALOGI("total_length is %x",total_length);
                    }else{
                        total_length = 16+pCode->insnsSize*2;
					//ALOGI("total_length is %x",total_length);
                    }
				
				    fwrite(Code_star,1,total_length,fp1);
				    fflush(fp1);

				    while(total_length&3){
	    ...
	    if (pClassData->virtualMethods) {
		    for (int i = 0; i < (int) pClassData->header.virtualMethodsSize; i++) {
			    ALOGI("virtualMethodsSize is %x i is %x method is %x",(int) pClassData->header.virtualMethodsSize,i,int(&pClassData->virtualMethods[i]));
			    //ALOGI("pDexFile addr is %x",int(pDexFile));
			    Method *method = &(clazz->virtualMethods[i]);
			    const DexCode* pCode = (DexCode*)((const u1*)method->insns-16);
			    uint8_t *Code_star=(uint8_t *)pCode;
			    pClassData->virtualMethods[i].codeOff = total_pointer;
			    //ALOGI("total_pointer is %x",total_pointer);
			    uint32_t ac = (method->accessFlags) & mask;
              	//ALOGI("GOT IT virtual method name %s.%s",descriptor,method->name);
			    if (!method->insns||ac&ACC_NATIVE) {
                    if (pClassData->virtualMethods[i].codeOff) {
                        pClassData->virtualMethods[i].accessFlags=ac;
	    ...
	    if(need_extra){
	        int class_def_len=0;
	        uint8_t *out=rewrite_ClassData(pClassData,class_def_len);
	        temp.classDataOff=total_pointer;
	        fwrite(out,1,class_def_len,fp1);
	        fflush(fp1);
	        while(class_def_len&3){
		        fwrite(&padding,1,1,fp1);
		        fflush(fp1);
		        class_def_len++;
	        }
	        total_pointer+=class_def_len;
	    }
	
	    fwrite(p,sizeof(DexClassDef),1,fp2);
	    fflush(fp2);
	}
}


void* Getdumppath(void* arg){
	FILE *file=NULL;
    while(dumppath[0]==0||dumppointer[0]==0){
		file=fopen("data/local/tmp/out","r");
		if(file==NULL){
			sleep(1);
			continue;
		}
		fgets(dumppath,99,file);
		dumppath[strlen(dumppath)-1]=0;
		fgets(dumppointer,99,file);
		dumppointer[strlen(dumppointer)-1]=0;
		fclose(file);
		file=NULL;
    }
	return NULL;
...
void* ExtractDEX(void* parament)
{
	
	DvmDex* methodClassDex=((struct arg*)parament)->pDvmDex;
  	Object *loader=((struct arg*)parament)->loader;
	char padding1=((struct arg*)parament)->padding1;
	int inc=((struct arg*)parament)->inc;
	Thread* self=((struct arg*)parament)->self;
	DexFile* dumpdexfile=methodClassDex->pDexFile;

	char *firstpart=new char[100];
	strcpy(firstpart,dumppath);
	strcat(firstpart,"/firstpart");

	char *datapart=new char[100];
	strcpy(datapart,dumppath);
	strcat(datapart,"/datapart");

	char *codeoffpart=new char[100];
	strcpy(codeoffpart,dumppath);
	strcat(codeoffpart,"/codeOff");
	FILE *fp1 = fopen(codeoffpart,"wb+");
	//ALOGI("Dumppath is %s",codeoffpart);

	char *classdefpart=new char[100];
	strcpy(classdefpart,dumppath);
	strcat(classdefpart,"/classdef");
	FILE *fp2 = fopen(classdefpart,"wb+");
	//ALOGI("Dumppath is %s",classdefpart);

	for (int i = 0; i < (int) dumpdexfile->pHeader->classDefsSize; i++) {
		//ALOGI("classDefsSize is %x i is %x",(int) dumpdexfile->pHeader->classDefsSize,i);
		getClassData(methodClassDex,i,fp1,fp2,loader,self);
	}
	fclose(fp1);
	fclose(fp2);
...
/*-------------------------------------reassamble--------------------------------*/
	...
	strcpy(result,dumppath);
	strcat(result,"/complete.dex");
	...
	char *resultdex=new char[100];
  	strcpy(resultdex,firstpart);

  	fd2=open(resultdex,O_RDONLY,0666);
  	if (fd2==-1) {
  	    ALOGI("open failed");
  	}
 	...
 	if(r==-1){  
  	...
  	len=st.st_size;
 	addr1=(char*)mmap(NULL,len,PROT_READ,MAP_PRIVATE,fd2,0);
 	fwrite(addr1,1,len,fp3);
  	fflush(fp3);
  	munmap(addr1,len);
  	close(fd2);

	char *result1=new char[100];
  	strcpy(result1,classdefpart);

  	fd2=open(result1,O_RDONLY,0666);
  	if (fd2==-1) {
  	    ALOGI("open failed");
  	}
	...
	while(inc>0){
		fwrite(&padding1,1,1,fp3);
		fflush(fp3);
		inc--;
	}

  	strcpy(result1,codeoffpart);
	...
	delete result1;
   	...
   	ALOGI("Dump finished!");
	char *chmod=new char[100];
	strcpy(chmod,"");
	strcat(chmod,"chmod 777 ");
	strcat(chmod,dumppath);
	strcat(chmod,"/*");
	ALOGI("chmod is %s",chmod);
	system(chmod);
...
/*------------------added begin-------------------------------*/
    ...
    if(uid){
    	if(readable){
		    ...
		    if(readable){
			    ...
			    pthread_t get_dumppath;
			    pthread_create(&get_dumppath,NULL,Getdumppath,NULL);
		    }else{
			    pthread_mutex_unlock(&read_mutex);
		    }
    	}
    ...
    char const *methodname=curMethod->clazz->descriptor;
    if(uid&&strcmp(dumppointer,"")){
        if(strstr(methodname,dumppointer)){
	        //ntpdate();
	        ALOGI("Dump Start!");
    	    //DexFile* dumpdexfile=curMethod->clazz->pDvmDex->pDexFile;
	        //ALOGI("Dex magic is %s",dumpdexfile->pOptHeader->magic);

    	    MemMapping *mem=&methodClassDex->memMap;
    	    const u1 *addr=(const u1*)mem->addr;

	        int length=int(mem->length);
	        //int baseaddr=int(dumpdexfile->baseAddr);
	        //ALOGI("Dex baseaddr is %x",baseaddr);
    	    //ALOGI("Dex length is %x",length);

	        char *firstpart=new char[100];
    	    strcpy(firstpart,dumppath);
	        strcat(firstpart,"/dumpeddex");
	        //ALOGI("Dumppath is %s",firstpart);
    	    FILE *fd=fopen(firstpart,"wb");
   	        fwrite(addr,sizeof(char),length,fd);
    	    fflush(fd);
   	        fclose(fd);
    ...
    if(uid&&strcmp(dumppointer,"")){
        char *res=strstr(methodname,dumppointer);
        if(res&&flag){
	        ...
	        if(flag){
	            ...
	            ALOGI("Dump Start!");
    	        DexFile* dumpdexfile=curMethod->clazz->pDvmDex->pDexFile;
	//ALOGI("oDex magic is %s",dumpdexfile->pOptHeader->magic);
	//ALOGI("Dex magic is %s",dumpdexfile->pHeader->magic);

    	        MemMapping *mem=&methodClassDex->memMap;
	            Object *loader=curMethod->clazz->classLoader;
    	        const u1 *addr=(const u1*)mem->addr;
	            total_pointer =int(mem->length)-0x28;

	            if(dumpdexfile->pOptHeader->magic==NULL){
		           total_pointer =int(mem->length);
	            }else{
		           total_pointer =int(mem->length)-0x28;
	            }
    	        ...
    	        int length=int(dumpdexfile->baseAddr+dumpdexfile->pHeader->classDefsOff-addr);
	//int baseaddr=int(dumpdexfile->baseAddr);
	//ALOGI("Dex baseaddr is %x",baseaddr);
	//ALOGI("oDex baseaddr is %x",int(addr));
    	//ALOGI("Dex length is %x",total_pointer);
	//ALOGI("Dumppath is %s",dumppath);

	            char *firstpart=new char[100];
	            strcpy(firstpart,dumppath);
	            strcat(firstpart,"/firstpart");
    	        FILE *fd=fopen(firstpart,"wb+");
                fwrite(addr,1,length,fd);
                fflush(fd);
                fclose(fd);
	//ALOGI("Dumppath is %s",firstpart);
	            ...
	            strcpy(datapart,dumppath);
	            strcat(datapart,"/datapart");
    	        FILE *fd1=fopen(datapart,"wb+");
	            addr=dumpdexfile->baseAddr+dumpdexfile->pHeader->classDefsOff+sizeof(DexClassDef)*dumpdexfile->pHeader->classDefsSize;
	            int datapartlen=int((const u1*)mem->addr+mem->length-addr);
   	            fwrite(addr,1,datapartlen,fd1);
    	        fflush(fd1);
   	            fclose(fd1);
	//ALOGI("Dumppath is %s",datapart);

	            param.loader=loader;
	            param.pDvmDex=methodClassDex;
	            param.padding1=padding1;
	            param.inc=inc;
	            param.self=self;

	            pthread_t Extrcatthread;
	            dvmCreateInternalThread(&Extrcatthread,"ClassDumper",ExtractDEX,(void*)&param);
	        }else{
		        pthread_mutex_unlock(&mutex);
	        }
        }
...
/*--------------------added end-------------------------------*/
    /*ALOGI("threadid=%d: %s.%s.%s pc=%#x fp=%p",
        self->threadId, curMethod->clazz->descriptor, curMethod->name,curMethod->clazz->sourceFile,
