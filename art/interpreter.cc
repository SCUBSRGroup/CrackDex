...
void* StartUpack(const DexFile &dex_file,mirror::ClassLoader *class_loader,ClassLinker *cl);

/*void* ReadThread(void* arg){
	const DexFile &dex_file=(*((struct arg*)arg)->dex_file);
  	mirror::ClassLoader *class_loader=((struct arg*)arg)->class_loader;
  	ClassLinker *cl = ((struct arg*)arg)->cl;

	FILE *file=NULL;
	LOG(INFO)<<"Enter readthread";
    	while(dexname[0]==0||dumppath[0]==0){
		file=fopen("data/local/tmp/ssss","r");
		if(file==NULL){
			sleep(1);
			continue;
		}
		fgets(dexname,99,file);
		dexname[strlen(dexname)-1]=0;
		LOG(INFO)<<"dexname is:"<<dexname;
		fgets(dumppath,99,file);
		dumppath[strlen(dumppath)-1]=0;
		LOG(INFO)<<"dumppath is:"<<dumppath;
		fclose(file);
		file=NULL;

		StartUpack(dex_file,class_loader,cl);
    	}
	return NULL;
}*/
...
void dexReadClassDataHeader(const uint8_t** pData,DexClassDataHeader *pHeader) {
...
void* DumpClass(void *parament)
{
  ...
  uint32_t start = dex_file.GetHeader().class_defs_off_ + sizeof(DexFile::ClassDef) * num_class_defs;
  ...
  for (size_t i=0;i<num_class_defs;i++) {
    ...
    if (pData->directMethods) {
      for (uint32_t i=0; i<pData->header.directMethodsSize; i++) {
	    ...
	    LOG(INFO)<<"zhixing qian method is"<<method;
	    const void* newcode=GetCompiledCodeToInterpreterBridge();
	    LOG(INFO)<<"newcode  is"<<method;
	    runtime->GetInstrumentation()->UpdateMethodsCode(method, newcode);
	    LOG(INFO)<<"zhixing hou method is"<<method;
        ...
        if (codeitem_off!=0) {
          ...
          pData->directMethods[i].codeOff = total_pointer;
	 	  MethodHelper mh(method);
          const DexFile::CodeItem * code = mh.GetCodeItem();
...
void* StartUpack(const DexFile &dex_file,mirror::ClassLoader *class_loader,ClassLinker *cl){
  ...
  if(flag){
      ...
      strcat(temp,"artdex");
	    LOG(INFO)<<"Dump path: "<<temp;
      FILE *fp = fopen(temp, "wb+");
      const byte *addr=dex_file.Begin();
 	    LOG(INFO)<<"dexfile begin addr is "<<dex_file.Begin();
      int length=dex_file.Size();
	    LOG(INFO)<<"dexfile length is "<<dex_file.Size();
      ...
      strcat(temp,"part1");
	    LOG(INFO)<<"Dump path: "<<temp;
      fp = fopen(temp, "wb+");
	    length=dex_file.GetHeader().class_defs_off_;
	    fwrite(addr,1,length,fp);
      fflush(fp);
      fclose(fp);
      ...
      strcat(temp,"data");
	    LOG(INFO)<<"Dump path: "<<temp;
      fp = fopen(temp, "wb+");
	    addr=dex_file.Begin()+dex_file.GetHeader().class_defs_off_+sizeof(DexFile::ClassDef)*dex_file.NumClassDefs();
      length=dex_file.Size()-dex_file.GetHeader().class_defs_off_-sizeof(DexFile::ClassDef)*dex_file.NumClassDefs();
      fwrite(addr,1,length,fp);
      fflush(fp);
      fclose(fp);
      delete temp;
...
static inline JValue Execute(Thread* self, MethodHelper& mh, const DexFile::CodeItem* code_item,
                             ShadowFrame& shadow_frame, JValue result_register) {
  const DexFile* dex_file=shadow_frame.GetMethod()->GetDeclaringClass()->GetDexCache()->GetDexFile();
  mirror::ClassLoader *class_loader=shadow_frame.GetMethod()->GetDeclaringClass()->GetClassLoader();
  std::string name(PrettyMethod(shadow_frame.GetMethod()));
  LOG(INFO)<<"Enter Exevute success ";
  LOG(INFO)<<"Got Method: "<<name;

  ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
  const DexFile& dexfile=*dex_file;
  strcpy(dexname,"com.example.seventyfour.tencenttest.MainActivity");
  strcpy(dumppath,"/data/data/com.example.seventyfour.tencenttest/");
  if(strstr(name.c_str(),dexname)&&start){
	start=false;
	LOG(INFO)<<"dump start!";
  	char * temp=new char[100];
        strcpy(temp,"/data/data/com.example.seventyfour.tencenttest/");
        strcat(temp,"noinit");
	LOG(INFO)<<"Dump path: "<<temp;
        FILE *fp = fopen(temp, "wb+");
        const byte *addr=dex_file->Begin();
        ...
        fclose(fp);
  	StartUpack(dexfile,class_loader,class_linker);
  }
...
extern "C" void artInterpreterToInterpreterBridge(Thread* self, MethodHelper& mh,
  ...
  std::string name(PrettyMethod(shadow_frame->GetMethod()));
  LOG(INFO)<<"Enter artInterpreterToInterpreterBridge success ";
  LOG(INFO)<<"Got Method: "<<name;
  ...
  if (LIKELY(!method->IsNative())) {
    ...
    std::string name(PrettyMethod(method));
    LOG(INFO)<<"Enter UnstartedRuntimeJni success ";
    LOG(INFO)<<"Got Method: "<<name;
    UnstartedRuntimeJni(self, method, receiver, args, result);
  }
